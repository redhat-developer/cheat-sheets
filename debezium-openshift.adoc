...
Debezium is built on top of Apache Kafka on `Kubernetes` and `OpenShift` with the Strimzi project. `Strimzi` provides a set of operators and container images for running Kafka on Kubernetes and OpenShift. 
